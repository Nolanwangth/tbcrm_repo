...
create table public.operation_cost_sheet_imports (
  id uuid primary key default gen_random_uuid(),
  case_id uuid not null references public.operation_cases(id) on delete cascade,
  source_file_id uuid not null references public.customer_files(id) on delete restrict,
  content_hash text not null check (content_hash ~ '^[0-9a-f]{64}$'),
  item_count integer not null check (item_count > 0),
  total_cost numeric(14,2) not null check (total_cost >= 0),
  total_quote numeric(14,2) not null check (total_quote >= 0),
...
alter table public.operation_days
  add column import_id uuid references public.operation_cost_sheet_imports(id) on delete set null;

alter table public.operation_service_items
  add column import_id uuid references public.operation_cost_sheet_imports(id) on delete set null,
  add column import_row_key text;

create index operation_cost_sheet_imports_case_created_idx
  on public.operation_cost_sheet_imports (case_id, created_at desc);

create unique index operation_service_items_import_row_key_idx
  on public.operation_service_items (case_id, import_row_key)
  ...
  loop
    ...
    if found then
      if v_existing_date is null then
        ...
        where id = v_day_id;
      elsif v_existing_date <> v_service_date then
...
alter table public.operation_cost_sheet_imports enable row level security;

revoke all on table public.operation_cost_sheet_imports
from public, anon, authenticated;

grant select, insert, update, delete on table public.operation_cost_sheet_imports
to service_role;
