...
interface Props {
    ...
    onSaveItemToLibrary: (item: QuoteItem, city: string) => PriceProduct | Promise<PriceProduct>;
    onUpdateLibraryProduct: (product: PriceProduct) => void | Promise<void>;
    onDeleteLibraryProduct: (id: string) => void | Promise<void>;
...
export function DirectQuotePage({ quote, setQuote, products, costsHidden, onToggleCosts, notify, onSaveItemToLibrary, onUpdateLibraryProduct, onDeleteLibraryProduct, groups, selectedGroupId, onSelectGroup, onCreateGroup, onRenameGroup, }: Props) {
    ...
    return (<main className={`workspace direct-workspace simplified-direct-workspace ${entryCollapsed ? "entry-collapsed" : ""} ${quoteCollapsed ? "quote-collapsed" : ""}`}>
          ...
          <section className={`direct-source-section user-library-section ${libraryOpen ? "open" : "closed"}`}>
            ...
            {libraryOpen && (userProducts.length ? <>
              ...
              {groupedLibraryProducts.length ? <div className="direct-library-groups">{groupedLibraryProducts.map((group) => <section key={group.id}><header><strong>{group.label}</strong><span>{group.items.length} 项</span></header><div className="direct-user-products">{group.items.map((product) => <article key={product.id}>
                ...
                <div className="direct-product-actions"><button onClick={() => setEditingProduct(product)} aria-label={`编辑${product.nameZh}`} title="编辑价格库项目"><Pencil size={13}/></button><button className="danger" onClick={async () => { if (!window.confirm(`删除价格库项目“${product.nameZh}”？\n不会删除当前报价中已经加入的项目。`))
                        return; try {
                        await onDeleteLibraryProduct(product.id);
                        notify(`已从价格库删除：${product.nameZh}`);
                    }
                    catch (error) {
                        notify(error instanceof Error ? error.message : "价格库删除失败，请重试");
                    } }} aria-label={`删除${product.nameZh}`} title="删除价格库项目"><Trash2 size={13}/></button></div>
      ...
      {editingProduct && <PriceProductEditModal product={editingProduct} onClose={() => setEditingProduct(null)} onSave={async (product) => { try {
        await onUpdateLibraryProduct(product);
        setEditingProduct(null);
        notify(`价格库项目已更新：${product.nameZh}`);
    }
    catch (error) {
        notify(error instanceof Error ? error.message : "价格库保存失败，输入已保留");
    } }}/>}
